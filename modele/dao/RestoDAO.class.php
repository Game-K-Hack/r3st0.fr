...
class RestoDAO {
     ...
     * @param string $voieAdrR nom de la rueM
    ...
    public static function getRestoByType(string $types): array {
        $lesObjets = array();
        try {
            $requete = "SELECT * FROM resto INNER JOIN typeparresto ON resto.idR = typeparresto.idR INNER JOIN type ON typeparresto.idTC = type.idTC WHERE type.libelleTC IN ('".$types."') GROUP BY nomR;";
            $stmt = Bdd::getConnexion()->prepare($requete);
            $ok = $stmt->execute();
            // attention, $ok = true pour un select ne retournant aucune ligne
            if ($ok) {
            ...
            throw new Exception("Erreur dans la méthode " . get_called_class() . "::getRestoByType : <br/>" . $e->getMessage());
        }
        return $lesObjets;
    }
